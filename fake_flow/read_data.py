...
def prepare_input(dataset='MultiSourceFake', segments_number=10, n_jobs=-1, emo_rep='frequency', return_features=True,
    ...
    content = pd.read_csv('/content/drive/MyDrive/Work_AsifaJunaid/fake_flow/data/{}/sample.csv'.format(dataset))
    content_features = []
    """Extract features, segment text, clean it."""
    if return_features:
        content_features = manual_features(n_jobs=n_jobs, path='/content/drive/MyDrive/Work_AsifaJunaid/fake_flow/features', model_name=dataset,
